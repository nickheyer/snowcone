//! PiSi backend for snowcone.
//!
//! Pardus's PiSi, the ancestor of Solus's eopkg, with the same verb set.
//! Mutations run through the elevation helper with `--yes-all` answering
//! prompts and a native `--dry-run` on install, remove, and upgrade. The
//! listing verbs (`list-installed`, `list-upgrades`, `search`) print
//! `name - summary` lines with no version column, so those results stay
//! version-less rather than paying one `info` call per package. `pisi
//! info` prints `Key : Value` stanzas under an "Installed package:" and a
//! "Package found in <repo> repository:" header, so info knows the
//! install state without a second probe. Versions render as
//! `version-release`, PiSi's own package identity.
...
impl Manager {
    ...
    /// (PiSi ships Turkish translations; `LC_ALL=C` keeps English).
    fn query(&self) -> Cmd {
...
/// PiSi has no version selection: installs always take the repository's
...
fn reject_pins(requests: &[PackageRequest]) -> Result<()> {
    match requests.iter().find(|request| request.version.is_some()) {
        Some(pinned) => Err(Error::Other(format!(
            "{ID}: `{pinned}` pins a version, but pisi always installs the repository's current version"
        ))),
        None => Ok(()),
    }
}
...
fn boxed(packages: Vec<PisiPackage>) -> Vec<Box<dyn Package>> {
    packages
        .into_iter()
        .map(|package| Box::new(package) as Box<dyn Package>)
        .collect()
...
fn parse_listing(stdout: &str, state: InstallState) -> Vec<PisiPackage> {
    stdout
        .lines()
        .filter_map(|line| {
...
/// `4.45 MB`-style sizes: PiSi divides by 1024 but labels KB/MB/GB.
...
/// `pisi info` stanzas: `Key : Value` fields under an `Installed
/// package:` and/or `Package found in <repo> repository:` header. The
/// `Name` line carries `name, version: X, release: Y` inline; versions
/// render as `X-Y` because the release is part of a PiSi package's
/// identity.
fn parse_info(stdout: &str) -> Option<PisiPackage> {
    let mut sections: Vec<(bool, Option<String>, PisiPackage)> = Vec::new();
    for line in stdout.lines() {
        let trimmed = line.trim();
        if trimmed == "Installed package:" {
            sections.push((true, None, PisiPackage::default()));
            continue;
        }
        ...
        {
            sections.push((false, Some(repo.to_string()), PisiPackage::default()));
            continue;
        }
    ...
    let mut installed: Option<PisiPackage> = None;
    let mut available: Option<(Option<String>, PisiPackage)> = None;
...
/// A package as PiSi describes it.
#[derive(Debug, Default)]
pub struct PisiPackage {
    pub name: String,
...
";
    ...
    fn parses_info_with_both_sections() {
        let stdout = "\
Installed package:
Name                : nano, version: 2.2.6, release: 24
...
Package found in Pardus-2011 repository:
Name                : nano, version: 2.2.6, release: 25
Summary             : Small, friendly text editor
Licenses            : GPLv3
Dependencies        : ncurses
Architecture        : i686
Package Size        : 512.00 KB
";
        let package = parse_info(stdout).unwrap();
        assert_eq!(package.name, "nano");
        assert_eq!(package.version.as_deref(), Some("2.2.6-24"));
        assert_eq!(package.latest_version.as_deref(), Some("2.2.6-25"));
        assert_eq!(package.state, InstallState::Upgradable);
        assert_eq!(package.origin.as_deref(), Some("Pardus-2011"));
        assert_eq!(package.dependencies, Some(vec!["ncurses".to_string()]));
        assert_eq!(package.installed_size, Some((1.6 * 1024.0 * 1024.0) as u64));
        assert_eq!(package.download_size, Some((512.0 * 1024.0) as u64));
    }
    ...
    fn parses_info_repo_only() {
        let stdout = "\
Package found in Pardus-2011 repository:
Name                : vim, version: 7.3, release: 45
Summary             : Advanced text editor
Licenses            : Vim
Architecture        : i686
Package Size        : 6.10 MB
";
        let package = parse_info(stdout).unwrap();
        assert_eq!(package.name, "vim");
        assert_eq!(package.version.as_deref(), Some("7.3-45"));
        assert_eq!(package.state, InstallState::Available);
        assert_eq!(package.license.as_deref(), Some("Vim"));
    }

    #[test]
    fn parses_sizes() {
        assert_eq!(parse_size("512.00 KB"), Some((512.0 * 1024.0) as u64));
        assert_eq!(parse_size("6.10 MB"), Some((6.1 * 1024.0 * 1024.0) as u64));
        assert_eq!(parse_size("nonsense"), None);
    }

    #[test]
    fn rejects_version_pins() {
        assert!(reject_pins(&[PackageRequest::parse("nano@2.2.6")]).is_err());
