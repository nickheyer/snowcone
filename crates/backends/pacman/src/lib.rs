//! pacman backend for snowcone.
//!
//! pacman is the baseline alpm CLI whose flag vocabulary the AUR helpers
//! mimic - same verbs, same output formats. Unlike the helpers, pacman
//! never escalates itself, so snowcone prefixes the elevation helper on
//! mutations. alpm keeps only the repo head, so version pins are rejected,
//! and pacman's `-Sp` preview resolves URLs without a real transaction, so
//! `--dry-run` errors instead of guessing.
...
/// alpm has no version selection: installs always take the repo head.
...
impl PackageManager for Manager {
    ...
    async fn install(&self, packages: &[PackageRequest], ctx: &OpContext) -> Result<()> {
        ...
        let mut cmd = self.cmd().arg("-S").arg("--needed").elevated(true);
        if ctx.assume_yes {
    ...
    async fn remove(&self, packages: &[PackageRequest], ctx: &OpContext) -> Result<()> {
        ...
        let mut cmd = self.cmd().arg("-R").elevated(true);
        if ctx.assume_yes {
            cmd = cmd.arg("--noconfirm");
        }
        cmd = cmd.args(packages.iter().map(|package| package.name.as_str()));
    ...
    async fn search(&self, query: &str) -> Result<Vec<Box<dyn Package>>> {
        let output = self
            ...
            .capture(&self.elevator, None)
            .await?;
        // pacman exits non-zero on "no matches".
    ...
    async fn refresh(&self, ctx: &OpContext) -> Result<()> {
        ...
        self.run(self.cmd().arg("-Sy").elevated(true), ctx).await
    }
...
fn parse_search(stdout: &str) -> Vec<Box<dyn Package>> {
    let mut packages: Vec<PacmanPackage> = Vec::new();
...
fn parse_info(stdout: &str, state: InstallState) -> Option<PacmanPackage> {
...
/// A package as pacman describes it.
...
core/bash 5.2.026-2 (base) [installed]
...
";
        ...
        assert_eq!(packages[2].name(), "ripgrep-all");
        assert_eq!(packages[2].state(), InstallState::Available);
        assert!(packages[2].description().unwrap().ends_with("tar.gz, etc."));
    }
