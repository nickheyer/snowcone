...
fn parse_search(stdout: &str) -> Vec<Box<dyn Package>> {
    let mut packages: Vec<ParuPackage> = Vec::new();
...
fn parse_info(stdout: &str, state: InstallState) -> Option<ParuPackage> {
...
/// A package as paru describes it.
#[derive(Debug, Default)]
pub struct ParuPackage {
    pub name: String,
