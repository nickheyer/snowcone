//! pikaur backend for snowcone.
//!
//! pikaur is a pacman-compatible AUR helper: repo and AUR packages live in the
//! same alpm database and share the pacman flag vocabulary. pikaur escalates
...
impl PackageManager for Manager {
    ...
    /// pikaur drives sudo itself for alpm mutations - snowcone never
...
fn parse_search(stdout: &str) -> Vec<Box<dyn Package>> {
    let mut packages: Vec<PikaurPackage> = Vec::new();
...
fn parse_info(stdout: &str, state: InstallState) -> Option<PikaurPackage> {
...
/// A package as pikaur describes it.
#[derive(Debug, Default)]
pub struct PikaurPackage {
    pub name: String,
...
impl Package for PikaurPackage {
    fn manager(&self) -> &str {
        ID
    }
