//! cpm backend for snowcone.
//!
//! cpm's default target is project-local, so snowcone explicitly selects
//! the active Perl installation with `--global`. Installed-module discovery
//! and removal use Perl's standard ExtUtils APIs against that same tree.
...
impl Manager {
    ...
    async fn installed(&self) -> Result<Vec<CpmPackage>> {
        let output = self
            .perl(LIST_SCRIPT)
...
impl PackageManager for Manager {
    ...
    async fn install(&self, packages: &[PackageRequest], ctx: &OpContext) -> Result<()> {
        ...
        self.run(
            self.cmd()
                .args(["install", "--global", "--progress=plain"])
...
fn parse_installed(stdout: &str) -> Vec<CpmPackage> {
    stdout
        .lines()
        .filter_map(|line| {
...
fn boxed(packages: Vec<CpmPackage>) -> Vec<Box<dyn Package>> {
    packages
        .into_iter()
        .map(|package| Box::new(package) as Box<dyn Package>)
        .collect()
...
/// A Perl module installed in the active global Perl library tree.
#[derive(Debug)]
