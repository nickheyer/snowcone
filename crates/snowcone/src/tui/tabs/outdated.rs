//! Outdated tab: packages with newer versions available, and the home of
//! upgrade-all.
...
pub fn draw(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let show_filter = ui::filter_bar_visible(app, Tab::Outdated);
    ...
    if let Some(filter_area) = filter_area {
        ui::draw_filter_bar(frame, app, Tab::Outdated, filter_area);
    }
...
fn draw_table(frame: &mut Frame, app: &mut App, area: Rect) {
    let tab = &app.outdated;
    let list = &tab.list;
    let title = format!(
        " Outdated ({}{}) - U upgrades everything ",
        list.visible_len(),
    ...
    if list.visible_len() == 0 {
        let text = match &tab.load {
            LoadState::NotLoaded | LoadState::Loading(_) => {
                format!("Checking for outdated packages… {}", ui::spinner(app.tick))
            }
            ...
            LoadState::Loaded => "Nothing outdated - everything is current.".to_string(),
        };
    ...
    let header = Row::new([
        ...
        Cell::from(format!("CURRENT{}", list.sort_indicator(SortKey::Version))),
        Cell::from("LATEST"),
    ...
    frame.render_stateful_widget(table, area, &mut app.outdated.list.table);
}
